...
- **Display & Headlines (Inter Tight):** Used for `display-lg` through `headline-sm`.
  - _Styling:_ Set with `-2%` to `-4%` letter spacing (tight tracking). This creates a "block" of text that feels architectural and premium.
- **Titles & Body (Inter):** Used for `title-lg` through `body-sm`.
  - _Styling:_ Standard tracking. Focus on generous line-height (`1.6` for body) to ensure the dark theme remains breathable.
- **Labels (Inter):** Used for `label-md` and `label-sm`.
  - _Styling:_ Uppercase with `+5%` letter spacing. This differentiates functional metadata from narrative content.
